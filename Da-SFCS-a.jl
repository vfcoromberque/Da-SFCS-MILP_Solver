...
function solver(
    ...
    ### Objective ###
    @objective(model, Max, sum(a[s] for s in S))
...
function runSolver()
    ...
    Total_Processing_Delay_s = Dict{Int,Int}() # Will store ceil(Float) as Int
    for s in S
        Total_Processing_Delay_s[s] = sum(p_ns[s, n] for n in N_s[s])
    end

    Total_Transmission_Delay_s = Dict{Int,Int}() # Will store ceil(Float) as Int
    for s in S
    ...
    Delta = collect(1:highest_value+1)

    println(u_s)
    println(p_ns)
    println(d_s)

    solver(S, K_p, L, F, t_f, c_ij, x, Delta, N_s, E_s, b_s, u_s, m_ns, p_ns, d_s, bw_cost, vnf_execution_cost)
end
